...
class ClassRepository(BaseRepository[Class]):
    ...
    def _create_dummy_instance(self) -> Class:
        return Class(id=0, name="", description="", id_teacher=0, id_students=[])

    def filter(self, name: str = None, id_teacher: int = None, id_student: int = None) -> list[Class]:
    ...
    def expand(self, classroom: Class):
        ...
        students = [self.student_repo.get_by_id(sid) for sid in classroom.id_students]
    ...
    def write_all(self, data: list[Class]) -> None:
        ...
        with open(self.csv_path, mode="w", newline="") as file:
            ...
            for item in data:
                ...
                row["id_students"] = "-".join(str(sid) for sid in row["id_students"])
                writer.writerow(row)
            ...
            with open(self.csv_path, mode="r", newline="") as file:
                ...
                for row in reader:
                    ...
                    if row["id_students"].strip() == "":
                        ...
                        row["id_students"] = [int(sid) for sid in row["id_students"].split("-") if sid.strip()]
