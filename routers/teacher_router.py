...
def get_teachers_csv_hash():
    hash_value = teacher_repo.calculate_csv_hash()
    if not hash_value:
        raise HTTPException(
...
def filter_teachers(
    ...
    return teacher_repo.filter(
        name=name, email=email, educational_background=educational_background
    )
...
def get_teacher(teacher_id: int):
    ...
    if not teacher:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail='Teacher not found'
        )
    return teacher
...
def create_teacher(teacher: Teacher):
    ...
    if not result:
        raise HTTPException(
            status_code=HTTPStatus.CONFLICT, detail='Teacher already exists'
        )
    return result
