...
def filter_classes(
    ...
    classes = class_repo.filter(name=name, id_teacher=id_teacher, id_student=id_student)
    return class_repo.expand_all(classes)
...
def create_class(classroom: Class):
    ...
    if not result:
        raise HTTPException(status_code=HTTPStatus.CONFLICT, detail="Class already exists")
    return class_repo.expand(result)

@router.put("/{class_id}", response_model=ClassExpanded, status_code=HTTPStatus.OK)
